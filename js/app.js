...
function U(p) {
  ...
  return ROOT + p.replace(/^\//, '').replace(/\/$/, '') + '.html' + tail;
}
...
var TG = (function () {
  ...
  function send(text) {
    var body = 'chat_id=' + encodeURIComponent(CHAT) +
...
function pcard(p) {
  ...
  return '<article class="pcard">' +
    ...
    '<div class="pcard__bottom">' + price +
      '<button class="btn btn--sm btn--block" data-add="' + p.slug + '" style="margin-top:10px">Добавить в корзину</button>' +
    '</div></article>';
}
...
function pageHome() {
  ...
  html += '<section class="section section--alt"><div class="container"><h2>Есть на складе</h2>' +
    '<div class="prod-grid">' + news.map(pcard).join('') + '</div></div></section>';
...
/* ---------------- page bindings ---------------- */
function bindPage() {
  // добавить в корзину (карточки)
  app.querySelectorAll('[data-add]').forEach(function (b) {
    b.addEventListener('click', function () {
      Cart.add(b.getAttribute('data-add'), 1);
  ...
  app.querySelectorAll('[data-auth-open]').forEach(function (b) {
    b.addEventListener('click', function (e) { e.preventDefault(); openAuthModal('reg'); });
  });
